...
module maindec(input        rst,
  ...
  localparam ExLS     = 4'b0100;   // Execute (Memory access address calculation)
  localparam ExB      = 4'b0101;   // Execute (Branch condition evaluation) and branch
  localparam WReg     = 4'b0110;   // Write ALU result back to the register
  localparam Mem_Read = 4'b0111;   // Perform memory read operation
  localparam Mem_Write = 4'b1000;  // Perform memory write operation
  localparam WRegL    = 4'b1001;   // Write loaded memory data back to the register
  localparam ExJal    = 4'b1010;   // Execute (PC + 4) and jump to the target address
  localparam ExJalr   = 4'b1011;   // Execute jump to the target address
  localparam WRegJr   = 4'b1100;   // Execute (PC + 4) and write back to the register
  localparam ExLui    = 4'b1101;   // Execute (U-type LUI instruction)
  /********** TODO: Add the states **********/
  ...
  always @(*) begin
    ...
    ExI: begin
      ...
      ALUOp <= 0; // No additional ALU operation
    end
    ExLui: begin
      controls <= 11'b11_01_00_00_000; // Load upper immediate (LUI) value into the register
      ALUOp <= 1; // Perform addition (0 + immediate)
    end
    ExJal: begin
      controls <= 11'b01_10_01_10_000; // Calculate the return address (OldPC + 4) and jump to the target address
      ALUOp <= 1; // Perform addition (OldPC + 4)
    end
    WReg: begin
      controls <= 11'b00_00_00_10_100; // Write ALU result to the destination register (rd)
      ALUOp <= 0; // No additional ALU operation
    end
    ExLS: begin
      controls <= 11'b10_01_00_11_000; // Calculate the memory access address, 
      ALUOp <= 1; // Perform addition (rs1 + immediate)
    end
    Mem_Read: begin
      controls <= 11'b00_00_00_10_000; // Select ALU output as memory address to read data
      ALUOp <= 0; // No additional ALU operation
    end
    WRegL: begin
    ...
    Mem_Write: begin
      controls <= 11'b00_00_00_10_010; // Select ALU output as memory address to write data
      ALUOp <= 0; // No additional ALU operation
    end
    ExB: begin
      controls <= 11'b10_00_00_10_001; // If branch condition is met, jump to the target address (calculated in Decode state)
      ALUOp <= 0; // Perform subtraction or comparison for branch condition
    end
    WRegJr: begin
      controls <= 11'b01_10_00_00_100; // Write the return address (PC + 4) to the destination register (rd)
      ALUOp <= 1; // Perform addition (PC + 4)
    end
    ExJalr: begin
      controls <= 11'b10_01_01_00_000; // Calculate the jump target address (rs1 + immediate) and jump
      ALUOp <= 1; // Perform addition (rs1 + immediate)
    end
    default: begin
  ...
  always @(*) begin
    ...
    Decode:	
    begin
      if (opcode == `OP_R) n_state <= ExR; // If opcode is R-type, transition to ExR (Execute R-type)
      ...
      else n_state <= IF; // Default: Transition back to IF state
    end
    ExLS:
    begin 	
...
module datapath(input         clk, reset_n, // clock and reset signals
  ...
  begin
    if (!reset_n)
      pc <= 0; //Reset the program counter (pc) to 0 when reset_n is low                                                              
  ...
  begin
    if      (ALUSrcB == 2'b00) alusrc2[31:0] = rs2_data_reg;
      ...
      else if ((opcode == `OP_I_Arith) || (opcode == `OP_I_Load)) alusrc2[31:0] = se_imm_itype[31:0]; // For I-type instructions (e.g., addi, lw), use the I-type immediate as the second ALU input.
	...
	begin
		...
		else rd_data[31:0] = 32'b0; // Default case: set rd_data to 0
	end
...
module RV32I (
  ...
  begin
    ...
    begin
      inst_reg <= inst; // //set inst_reg to inst when IRWrte signal was enable
      OldPC <= pc; //set OldPC to pc when IRWrte signal was enable
    end
  end
  ...
  begin
    ...
    else MemRdata_reg <= MemRdata; // set MemRdata register to MemRdata
  end
